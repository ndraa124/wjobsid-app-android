...
import com.id124.wjobsid.activity.main.fragment.hiring.fragment_hiring.reject.adapter.RejectHireAdapter
import com.id124.wjobsid.base.BaseFragmentCoroutine
import com.id124.wjobsid.databinding.FragmentRejectBinding
import com.id124.wjobsid.model.hire.HireModel

class RejectFragment : BaseFragmentCoroutine<FragmentRejectBinding>(), RejectContract.View {
    private var presenter: RejectPresenter? = null

    override fun onCreate(savedInstanceState: Bundle?) {
        setLayout = R.layout.fragment_reject
        super.onCreate(savedInstanceState)
    }

    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        super.onViewCreated(view, savedInstanceState)
        presenter = RejectPresenter(createApi(activity))

        setHiringProjectRecyclerView()
    }

    override fun onResultSuccess(list: List<HireModel>) {
        (bind.rvHire.adapter as RejectHireAdapter).addList(list)
    ...
    private fun setHiringProjectRecyclerView() {
        ...
        val adapter = RejectHireAdapter()
        bind.rvHire.adapter = adapter

        adapter.setOnItemClickCallback(object : RejectHireAdapter.OnItemClickCallback {
